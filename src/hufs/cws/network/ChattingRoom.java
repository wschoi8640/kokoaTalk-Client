...
public class ChattingRoom extends VBox
{
		...
		void handleConnect() 
		{
				try
				{
			            ...
			            sock = new Socket("172.30.1.42", 10002);
			            
			            new EchoClientThread(sock).start();		
			    }
				catch (IOException e) 
				{
						e.printStackTrace();
		...
		private class EchoClientThread extends Thread
		{
				Socket sock;
				
				EchoClientThread(Socket sock)
				{
						this.sock = sock;
				}
				@Override
						...
						try 
						{
					            ...
					            while(true)
								{
										...
										for(String messageInfo : messageInfos)
										{
												...
												{
														if(messageInfo.equals(chatroomInfo)) count++;
